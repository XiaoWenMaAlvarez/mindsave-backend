...
export class EmailVerificationSender {
  ...
  async execute(email: string, name: string): Promise<void> {
    const token = JwtAdapter.generateToken({ email }, "email-verification", "24h");
    ...
    await this.sendEmailValidationLink(email, verifyEmailUrlWithToken, name);
  }

  private async sendEmailValidationLink(email: string, link: string, name: string): Promise<void> {
